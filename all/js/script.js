...
function initEndScrollWatcher() {
  ...
  // === НАЛАШТУВАННЯ ===
  // 'vh' -> trigger коли scrollY >= window.innerHeight * END_TRIGGER_VH
  const END_TRIGGER_MODE = 'vh'; 
  const END_TRIGGER_VH = 1.25;   // <- Вибране значення: 125% від viewport
  ...
  function onScrollTick() {
    ...
    // Визначаємо "nearTrigger" в залежності від режиму
    let nearTrigger = false;
    if (END_TRIGGER_MODE === 'vh') {
      // коли прокручено більше ніж N * viewport height
      nearTrigger = (y >= (winH * END_TRIGGER_VH));
    } else {
      // режим 'bottom' — базується на відстані від низу
      nearTrigger = (winH + y) >= (docH - CONFIG.end.growThresholdPxFromBottom);
    }

    // Основна логіка — якщо ми "досягли" точки та скролимо вниз -> grow
    if (nearTrigger && directionDown && !end.isActive()) {
      end.grow();
      return;
    }

    // Якщо ми відійшли вгору поза поріг (гістерезис) — shrink
    const hysteresisPx = CONFIG.end.hysteresis || 60;
    let aboveThreshold = false;
    if (END_TRIGGER_MODE === 'vh') {
      aboveThreshold = (y < (winH * END_TRIGGER_VH) - hysteresisPx);
    } else {
      aboveThreshold = (winH + y) < (docH - (CONFIG.end.growThresholdPxFromBottom + hysteresisPx));
    }
...
/* ==== end-cover-toggle.js ==== 
   Вставити в кінець ../main/all/js/script.js
   Призначення: додати клас body.end-covered коли #end-circle повністю покриває екран,
   і прибрати цей клас коли користувач прокрутить вгору за поріг.
*/
(function () {
  'use strict';

  const CIRCLE_ID = 'end-circle';
  const BODY_CLASS = 'end-covered';

  // збільшимо поріг від нижньої межі (має відповідати CONFIG.end.growThresholdPxFromBottom)
  const GROW_THRESHOLD_PX = (function() {
    // якщо CONFIG доступен глобально — бери з нього, інакше дефолт 120
    try { return (window.CONFIG && window.CONFIG.end && window.CONFIG.end.growThresholdPxFromBottom) || 120; } catch(e) { return 120; }
  })();

  function circleCoversViewport(circleEl) {
    if (!circleEl) return false;
    const r = circleEl.getBoundingClientRect();
    // трохи запасу для числових похибок
  ...
  function setCoveredState(on) {
    if (on) {
      if (!document.body.classList.contains(BODY_CLASS)) document.body.classList.add(BODY_CLASS);
    } else {
      if (document.body.classList.contains(BODY_CLASS)) document.body.classList.remove(BODY_CLASS);
    }
  }

  // Основна перевірка: якщо коло покрило viewport => встановити end-covered.
  // І навпаки, якщо ми підійшли вгору від низу на достатню відстань — зняти.
  function checkState(circleEl) {
    if (!circleEl) return;
    if (circleCoversViewport(circleEl)) {
      setCoveredState(true);
      return;
    }
    // якщо коло не покриває — перевіримо положення скролу: якщо ми вже відійшли від низу більше порога — зняти
    const y = window.scrollY || window.pageYOffset || 0;
    const winH = window.innerHeight;
    const docH = document.documentElement.scrollHeight || document.body.scrollHeight;
    const nearBottom = (winH + y) >= (docH - GROW_THRESHOLD_PX);
    if (!nearBottom) setCoveredState(false);
  }

  // шукаємо circle; якщо ще немає — використовуємо MutationObserver
  function waitForCircleAndInit() {
    const circleEl = document.getElementById(CIRCLE_ID);
    if (circleEl) {
      initListeners(circleEl);
      return;
    }
    const mo = new MutationObserver((mutations, obs) => {
      const found = document.getElementById(CIRCLE_ID);
      if (found) {
        obs.disconnect();
        initListeners(found);
      }
  ...
  function initListeners(circleEl) {
    // реагуємо на завершення трансформації (коли коло доросте до кінцевого масштабу)
    circleEl.addEventListener('transitionend', (ev) => {
      if (!ev.propertyName || ev.propertyName.toLowerCase().indexOf('transform') === -1) return;
      // після закінчення трансформу перевіряємо чи коло насправді покрило viewport
      checkState(circleEl);
    });

    // також слухаємо scroll/resize — щоб зняти стан коли користувач прокрутить вгору
    let ticking = false;
    function onScrollTick() {
      ticking = false;
      checkState(circleEl);
    ...
    window.addEventListener('resize', () => checkState(circleEl), { passive: true });

    // стартова перевірка (на випадок, якщо сторінка вже внизу)
    requestAnimationFrame(() => checkState(circleEl));
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', waitForCircleAndInit);
  } else {
    waitForCircleAndInit();
  }

  // DEBUG API
  window.__endCoverDebug = {
    setOn: () => document.body.classList.add(BODY_CLASS),
    setOff: () => document.body.classList.remove(BODY_CLASS),
    isOn: () => document.body.classList.contains(BODY_CLASS)
  };
})();
